...
func AddSV(f *File, fileName, typeName string, kvPair map[string]string) error {
	...
	/* First we sort the kvPair's key */
	keys := make([]string, 0, len(kvPair))
	for k := range kvPair {
		keys = append(keys, k)
	}
	sort.Strings(keys)
	...
	f.Func().Params(
		...
		Switch(Id("v").Op(":=").Id("i").Assert(Type())).Block(
			...
			Default().Block(
				Return(Qual("github.com/pkg/errors", "Errorf").Call(
		...
		Switch(Id("vv")).BlockFunc(func(g *Group) {
			...
			g.Default().Block(
				Return(
					Qual("github.com/pkg/errors", "Errorf").Call(Lit(fmt.Sprintf("Invalid value of type %s: %%v", typeName)), Id("vv")),
				),
			)
		}),
	...
	f.Func().Params(
		...
		Switch(Id("t")).BlockFunc(func(g *Group) {
			...
			g.Default().Block(
				Return(
					Nil(),
					Qual("github.com/pkg/errors", "Errorf").Call(Lit(fmt.Sprintf("Invalid value of type %s: %%v", typeName)), Id("t")),
				),
			)
		}),
